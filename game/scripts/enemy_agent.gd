...
func tester_prepare_search_state(setup_generation: int) -> Dictionary:
	...
	receipt.merge({
		...
		"animation": {
			...
			"weapon_socket_bound": presentation_state.get("weapon_socket_bound", presentation_state.get("socket_bound", false)),
			"weapon_socket_contact_status": presentation_state.get("weapon_socket_contact_status", &"unreported"),
			"rifle_contact": presentation_state.get("rifle_contact", {}),
		},
